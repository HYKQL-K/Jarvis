...
float ComputeRmsScore(const int16_t* pcm, int n) {
    ...
    // Normalize to 0..1 relative to int16 max
...
extern "C" int jarvis_wake_init(const char* model_path, float sensitivity) {
    ...
    // Clamp sensitivity into [0.1, 0.95]
    const float s = std::fmax(0.1f, std::fmin(0.95f, sensitivity));
    g_wake_ctx->high_threshold = 0.35f * s + 0.15f; // 0.185..0.4825
    ...
    // Placeholder: In a real integration, load openWakeWord model here using model_path.
...
extern "C" int jarvis_wake_score(const int16_t* pcm, int n, float* score) {
    ...
    // TODO: Replace RMS scoring with openWakeWord model inference when available.
    ...
    // Double-threshold logic: must cross high, then sustain above low for linger_needed frames
    ...
    if (g_wake_ctx->armed) {
        if (s >= g_wake_ctx->low_threshold) {
            ...
            if (g_wake_ctx->post_high_frames >= g_wake_ctx->linger_needed &&
                g_wake_ctx->cooldown_frames == 0) {
                g_wake_ctx->armed = false;
                g_wake_ctx->cooldown_frames = 10; // short refractory to reduce repeats
                return 1; // fired
            }
        } else {
            // Drop below low threshold: reset the linger counter but stay armed until cooldown finishes
            g_wake_ctx->post_high_frames = 0;
        }
    }
